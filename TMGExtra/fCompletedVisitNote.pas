...
uses uTMGMods, uUtility, fCosign, rRPCs, fSignItem, fCompleteConsult;
...
begin
  HTMLEditor.Print(True);
end;

procedure TfrmCompletedVisitNote.btnDoneClick(Sender: TObject);
begin
  //if ckbPrintAfterSig.checked then HTMLEditor.Print(True);
...
begin
  ...
  with FAppState do begin
    ...
    try
      if frmSignItem.ShowModal('Sign Document', 'Electronic Signature Code') = mrOK then begin
        if ckbPrintAfterSig.Checked then begin
          HTMLEditor.Print;
          MessageDlg('Press OK when done printing.', mtInformation, [mbOK], 0);
        end;
        CosignWanted := frmSignItem.CosignerWanted;
        EncSig := frmSignItem.ESCode;
      end;

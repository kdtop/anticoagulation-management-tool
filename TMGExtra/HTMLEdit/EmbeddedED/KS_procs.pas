...
const
  ...
  EIGHT = '8';
  NINE = '9';
  //kt DIGITS: set of Char = [ZERO..NINE];
  DIGITS: set of WideChar = [ZERO..NINE];
...
//kt to achieve compile -- Function GetFileDateTime(aFile: string): String;
//kt to achieve compile -- Function GetShortDateTime(aTime: TDateTime; Seconds: boolean = false): String;
...
//kt to achieve compile -- procedure DeveloperMessage(aMessage: string);
function CloseDeveloperMessagesLog(afile: string): boolean;
function SaveDeveloperMessagesLog(afile: string): boolean;
//kt to achieve compile -- procedure OpenDeveloperMessagesLog;
...
{ date functions }
//kt to achive compile -- Function DateStrToDateTime(aDate: string): TDateTime;
//kt to achive compile -- Function TimeStrToDateTime(aDateTime: string): TDateTime;
...
//kt to achive compile --- Function GetFileAsText(const afile: String): String;
...
begin
  ...
  //kt if (length(s) > 0) and (s[1] in ['+','-','0'..'9'])
  if (length(s) > 0) and CharInSet(s[1], ['+','-','0'..'9'])  //kt
     then begin
        for a := 2 to length(s) do
           //kt if not (s[a] in ['0'..'9'])
           if not CharInSet(s[a], ['0'..'9'])  //kt
              then begin
                 if (a > 3) or //min two good numbers before the trird that is bad
                    //kt ((a = 2) and (not (s[1] in ['+','-']))) //first number is good
                    ((a = 2) and (not CharInSet(s[1], ['+','-']))) //first number is good  //kt
...
begin
  //asm int 3 end; //KS trap
  //kt while (Pos <= Length(S)) and not (S[Pos] in ['0'..'9']) do
  while (Pos <= Length(S)) and not CharInSet(S[Pos], ['0'..'9']) do  //kt
  begin
    //kt if S[Pos] in LeadBytes then Inc(Pos);
    if CharInSet(S[Pos], LeadBytes) then Inc(Pos);  //kt
    Inc(Pos);
  end;
end;
//------------------------------------------------------------------------------
...
begin
  ...
  Result := 0;
  //kt mod ---
  for I := Low(FormatSettings.EraInfo) to High(FormatSettings.EraInfo) do begin
    EraName := FormatSettings.EraInfo[I].EraName;
    if EraName = '' then break;
    if Pos(EraName, Name) > 0 then begin
      //Result := EraYearOffsets[I];
      Result := FormatSettings.GetEraYearOffset(EraName);
      Exit;
    end;
  end;
  //kt end mod ---
...
begin 
  ...
  I := Pos;
  N := 0;
  //kt while (I <= Length(S)) and (S[I] in ['0'..'9']) and (N < 1000) do
...
begin 
  //asm int 3 end; //KS trap
  //kt to achive compile --- Result := Trim(GetFileAsText(GetWinDir + 'KSUserCookie'));
  Result := ''; //kt
end;
//------------------------------------------------------------------------------
var
...
begin
  ...
  for Index := 1 to Length(Search) do begin
    if InString or (Search[Index] in BlackSpace) then
      //kt AppendStr(Result, Search[Index]);
      Result := Result + Search[Index];  //kt

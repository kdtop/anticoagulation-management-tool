...
//kt to achieve compile -- Function ReadRegString(MainKey: HKey; SubKey, ValName: String): String;
//kt to achieve compile -- Procedure WriteRegString(MainKey: HKey; SubKey, ValName: String; const Data: String);
//kt to achieve compile -- function GetExeOpen(Ext: string; var Exefil, Params: string; sielent: boolean = true): Boolean;
//kt to achieve compile -- procedure ExecuteDefaultOpen(ext, aFile: String);
...
// the best way to find an exe is through the registry entries - there we get any
//  command line param that might be neded, but if it fails we can try FindExecutable
//  (which is in the center og the _GetExeOpen function), but it only returns the
//  exe file
var
  S: string;
  I: integer;
  ...
  begin
     ...
     //now we have an exefile and it can have some params starting with " /" or " -"
     I := Pos(' /', Exefil);
     if I > 0
        then begin
           //we have params

...
type
  ...
  //kt TMessageEventEx = procedure(Sender: TObject; var msg: LongWord; var wParam: SYSINT; var lParam: SYSINT; var Result: SYSINT) of object;
  TMessageEventEx = procedure(Sender: TObject; var msg: Cardinal; var wParam: WPARAM; var lParam: LPARAM; var Result: LRESULT) of object;
...
//kt  TSnapRect = procedure(Sender: TObject; const pIElement: IHTMLElement; var prcNew: tagRECT; eHandle: _ELEMENT_CORNER; var Result: HResult) of object;
  ...
  private
    ...
    //kt function LoadFromString(aString: String): HResult;
    ...
    //kt procedure SetDocumentHTML(NewHTML: String);
  ...
  protected
    ...
    //kt function GetDocumentHTML: string;
  ...
  public
    ...
    procedure Refresh;
    //kt property DocumentHTML: String read GetDocumentHTML write SetDocumentHTML;
...
uses SysUtils, dialogs, FileCtrl, ComObj,
     ...
     UEditHost, KS_Procs, KS_Procs2, IEDispConst {, RegFuncs} ;  //kt
...
begin
  ...
  if length(FIEVersion) > 0 then begin
    FIE6 := FIEVersion[1] >= '6';
    if (not FIE6) then begin
      if (FIEVersion[1] < '5') or (FIEVersion[3] < '5') then
        KSMessageE('This HTML-editor Component '+CrLf+'need IE 5.5 or higher');
    end;
  end;
  }
...
var
  ...
  aStream: IStream;
{$ENDIF}
  SingleByteStr : AnsiString; //kt 2/21/18
begin
  //asm int 3 end; //trap

  FLoadFromString := true;

  SingleByteStr := aString; //might result in loss of unicode chars.  //kt 2/21/18
  ...
  {$ELSE}
     ...
     try
        if aHandle <> 0
           then begin
              //kt Move(aString[1], PChar(aHandle)^, Length(aString) + 1);
              Move(SingleByteStr[1], PChar(aHandle)^, Length(SingleByteStr) + 1); //kt 2/21/18

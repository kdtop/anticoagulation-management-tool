...
begin
 if not (UntypedAppState is TAppState) then exit;
 AppState := TAppState(UntypedAppState);
 SaveFlowsheetToServer(self, AppState);
end;
...
begin
  Result := StrToFloatDef(Trim(piece(CurrentINRGoal,'-',2)),0);
end;

function TOneFlowsheet.GetfINRGoalLow : single;
begin
  Result := StrToFloatDef(Trim(piece(CurrentINRGoal,'-',1)),0);
end;

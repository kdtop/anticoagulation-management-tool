...
type
  //kt TCharacterSet = Set of Char;
  TFMDateTime = Double;
  TORIdleCallProc = procedure(Msg: string);
{ sundries }
//kt function CharInSet(AChar: Char; ASetOfChar: TCharacterSet) : Boolean;
...
//procedure FastAssign(source, destination: TStrings);     //kt-cp
//procedure UpdateReadOnlyColorScheme(Control: TControl; ReadOnly: boolean);    //kt-cp
//procedure FastAddStrings(source, destination: TStrings);  //kt-cp
...
begin {FormatFMDateTime}
  ...
  while Length(AFormat) > 0 do
    ...
    '"': begin                                                                 // literal
           Delete(AFormat, 1, 1);
           //kt while not (CharAt(AFormat, 1) in [#0, '"']) do
           while not CharInSet(CharAt(AFormat, 1), [#0, '"']) do  //kt
           begin
...
begin
  ...
  //kt for i := 1 to 7 do if not (x[i] in ['0'..'9']) then Exit;
  for i := 1 to 7 do if not CharInSet(x[i], ['0'..'9']) then Exit;  //kt
  if (Length(x) > 7) and (x[8] <> '.') then Exit;
  //kt if (Length(x) > 8) and not (x[9] in ['0'..'9']) then Exit;
  if (Length(x) > 8) and not CharInSet(x[9], ['0'..'9']) then Exit;  //kt
  Result := True;
end;
...
begin
  Result := False;
  //kt for i := 1 to Length(x) do if x[i] in ['A'..'Z','a'..'z'] then
...
begin
  Result := False;
  //kt for i := 1 to Length(x) do if x[i] in ['!'..'~'] then  // ordinal values 33..126
...
begin
  ...
  for i := 1 to Length(x) do begin
    ...
    //kt #128..#159: Result := Result + '?';
    char(128)..Char(159): Result := Result + '?';
...
var
  i: Integer;
begin
  //kt while (Length(x) > 0) and not (x[1] in ['0'..'9']) do Delete(x, 1, 1);
  while (Length(x) > 0) and not CharInSet(x[1], ['0'..'9']) do Delete(x, 1, 1);  //kt
  //kt for i := 1 to Length(x) do if not (x[i] in ['0'..'9']) then break;
  for i := 1 to Length(x) do if not CharInSet(x[i], ['0'..'9']) then break;  //lt
...
begin
  ...
  while (Length(x) > 0) and not CharInSet(x[1], ['0'..'9', '.']) do Delete(x, 1, 1); //kt
  //kt for i := 1 to Length(x) do if not (x[i] in ['0'..'9','.']) then break;
  for i := 1 to Length(x) do if not CharInSet(x[i], ['0'..'9','.']) then break;  //kt
...
begin
  ...
  for i := 2 to Length(x) do
     //kt if (not (x[i-1] in [' ',',','-','.','/','^'])) and (x[i] in ['A'..'Z'])
     if (not CharInSet(x[i-1], [' ',',','-','.','/','^'])) and CharInSet(x[i], ['A'..'Z'])  //kt
 // save line    if (not (x[i-1] in [' ','''',',','-','.','/','^'])) and (x[i] in ['A'..'Z'])
      then Result[i] := Chr(Ord(x[i]) + 32)
     //kt else if ((x[i-1] in [' ',',','-','.','/','^'])) and (x[i] in ['a'..'z'])
...
function FindPiecesNodes(SL : TStringList; Delim : char; NodeA : string = ''; NodeB : string = ''; NodeC : string = ''; nodeD : string = '') : integer;
//kt added function 5/15
//Purpose: search for index in SL that matches input value(s)
//Input: SL -- standard TStringList, but every line expected to be in format  text^text^text^text^....
//       NodeA...NodeB -- these are values to search for to match.  If '' passed in, then that
//                        node position is not used for searching.  At least one of Node* must be specified
//Result: -1 if not found, or index number if match is found.
var i : integer;
    s : string;
begin
  Result := -1;
  if not assigned(SL) then exit;
  if (NodeA = '') and (NodeB = '') and (NodeC = '') and (NodeD = '') then exit;
  for i := 0 to SL.Count - 1 do begin
    s := SL.Strings[i];
    if (NodeA <> '') and (Piece(s, Delim, 1) <> NodeA) then continue;
    if (NodeB <> '') and (Piece(s, Delim, 2) <> NodeB) then continue;
    if (NodeC <> '') and (Piece(s, Delim, 3) <> NodeC) then continue;
    if (NodeD <> '') and (Piece(s, Delim, 4) <> NodeD) then continue;
    Result := i;
    break;
  end;
end;
...
function FindPiece(SL : TStringList; Delim : char; PieceNum : Integer; Value : string) : integer;  //kt 6/15 added
//kt added function 6/15
//Purpose: search for index in SL that matches input value
//Result: -1 if not found, or index number if match is found.
var i : integer;
    s : string;
begin
  Result := -1;
  if not assigned(SL) then exit;
  if PieceNum < 1 then exit;
  for i := 0 to SL.Count - 1 do begin
    s := SL.Strings[i];
    if Piece(s, Delim, PieceNum) <> Value then continue;
    Result := i;
    break;
  end;
end;
...
function Pieces2(const S: string; Delim: string; PieceStart,PieceEnd: Integer): string;
//kt 8/09 Added entire function
...
//kt 11/13 added
begin
  Result := VAUtils.Piece2(S, Delim, PieceNum);
end;

function Contains(const S, SubString : string) : boolean;
//kt added 7/1/16
begin
  Result := Pos(SubString, S) > 0;
end;

function LeftMatches(S, SubString : string; CaseSensitive : boolean = true) : boolean;
//kt added 7/1/16
var L : integer;
begin
  Result := false;
  L := length(SubString);
  if L > length(S) then exit;
  if not CaseSensitive then begin
    S := Uppercase(S);
    SubString := Uppercase(SubString);
  end;
  Result := (LeftStr(S, L) = SubString);
end;

function RightMatches(S, SubString : string; CaseSensitive : boolean = true) : boolean;
//kt added 7/1/16
var L : integer;
begin
  Result := false;
...
//kt added 6/15
//E.g.   LeftMatch('This','This is a test') ==> true
begin
  Result := (Pos(SubStr, Str) = 1) and (Length (SubStr) <= Length(Str));
end;


function RightMatch(SubStr, Str : string) : boolean;
//kt added 6/15
//E.g.   RightMatch('test','This is a test') ==> true
begin
...
procedure PiecesToList2(x: string; ADelim: String; AList: TStrings);
{ adds each piece to a TStrings list, the list is cleared first }
//kt added 2/21/16
var
  APiece: string;
...
begin
  ...
  begin
    //kt if Result[i] in ['a'..'z','A'..'Z','0'..'9',#32] then
...
{//kt, for use of function in SysUtils

...
type
  ...
  protected
    ...
    //kt procedure YearUDChange(Sender: TObject; var AllowChange: Boolean;
    //kt                        NewValue: Smallint; Direction: TUpDownDirection);
...
begin
  ...
  begin
    ...
    begin
      if FLongMonths then
        //ktFMonthCombo.Items.Add(LongMonthNames[i])
        FMonthCombo.Items.Add(FormatSettings.LongMonthNames[i])  //kt
      else
        //kt FMonthCombo.Items.Add(ShortMonthNames[i]);
        FMonthCombo.Items.Add(FormatSettings.ShortMonthNames[i]);  //kt
      if(GetSize) then
      begin
...
//kt procedure TORDateCombo.YearUDChange(Sender: TObject; var AllowChange: Boolean;
//                                      NewValue: Smallint; Direction: TUpDownDirection);
procedure TORDateCombo.YearUDChange(Sender: TObject; var AllowChange: Boolean;
                                    NewValue: Integer; Direction: TUpDownDirection);  //kt
var
  y, m, d: word;
...
begin
  //kt if(Key in ['0'..'9']) and (FYearEdit.Text = '    ') then
  if CharInSet(Key, ['0'..'9']) and (FYearEdit.Text = '    ') then  //kt
  begin
    FYearEdit.Text := Key + '   ';
    Key := #0;
...
begin
  ...
  begin
    ...
    begin
      if FLongMonths then
        //kt Result := LongMonthNames[FMonth]
        Result := FormatSettings.LongMonthNames[FMonth]  //kt
      else
        //kt Result := ShortMonthNames[FMonth];
        Result := FormatSettings.ShortMonthNames[FMonth];  //kt
      if(FDay > 0) then
